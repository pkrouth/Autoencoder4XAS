...
class XasLinEncoders(pl.LightningModule):
    ...
    def prepare_data(self):
        transformed_dataset = XDataset(json_file=self.hparams.json_file,
                                root_dir='../data/',
                                transform=Compose([Normalize() ,ToTensor()]))##off
        self.train_size = int(0.98*len(transformed_dataset))
        self.val_size = len(transformed_dataset)-self.train_size
        self.train_ds, self.val_ds = random_split(transformed_dataset,[self.train_size,self.val_size])
        #self.train_ds, self.val_ds = self.train_ds.float(), self.val_ds.float()
        
        
    @pl.data_loader  
    def train_dataloader(self):
        return DataLoader(self.train_ds, batch_size=self.hparams.batch_size, num_workers=self.hparams.cpus)
    
    @pl.data_loader
    def val_dataloader(self):
        return DataLoader(self.val_ds, batch_size=self.hparams.batch_size, num_workers=self.hparams.cpus)
...
class XasConvAutoEncoder2(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        #self.latent_size = latent_size
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=1,out_channels=8,kernel_size=3,stride=2,padding=1), # Input: (bs,nc=1,l=100) Output: (bs,8,50)
            nn.ReLU(True),
            nn.AvgPool1d(kernel_size=3, stride=1), # Output: #bs, 8, 48
            nn.Conv1d(in_channels=8, out_channels=16, kernel_size=2, stride=2, padding=1),  # Output: b, 16, 25
            nn.ReLU(True),
            nn.AvgPool1d(kernel_size=3, stride=2),  # Output: b, 16, 12
            nn.Conv1d(in_channels=16, out_channels=32, kernel_size=6, stride=2, padding=1),  # Output: b, 32, 5
            nn.ReLU(True),
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = x.float()
        reconstructed = self.forward(x)
        loss = nn.L1Loss()(reconstructed, x)#, reduction='mean')##Removing reduce here...
        tensorboard_logs = {'train_loss':loss}
        return {'loss':loss, 'log':tensorboard_logs}
...
class XasLinAutoEncoder2(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.hparams = hparams
        self.encoder = nn.Sequential(
            nn.Linear(10 * 10, 128),nn.Tanh(),
            nn.Linear(128, 64),nn.Tanh(),
            nn.Linear(64, 12), nn.Tanh(),
            nn.Linear(12, hparams.latent_size))
        self.decoder = nn.Sequential(
            nn.Linear(hparams.latent_size, 12),nn.Tanh(),
            nn.Linear(12, 64),nn.Tanh(),
            nn.Linear(64, 128),nn.Tanh(),
            nn.Linear(128, 10 * 10)) ## Removing tanH from the last layer 
