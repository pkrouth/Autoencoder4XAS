...
class VaeMmdConv3Lin1(pl.LightningModule):
    ...
    def prepare_data(self):
        train_dataset = XDataset(json_file=self.hparams.json_file,
                                root_dir='../data/',
        ...
        if self.hparams.test_json_file:
            self.test_ds = XDataset(json_file=self.hparams.test_json_file,
                                root_dir='../data/',
                                transform=Compose([ToTensor()]))
    ...
    def training_step(self, batch, batch_idx):
        ...
        tensorboard_logs = {'train_loss':loss, 'train_recon_loss':reconstruction_error, 'train_mmd_loss':mmd}
    ...
    def training_epoch_end( self, outputs):
        ...
        train_recon_loss_epoch = torch.stack([x['train_recon_loss'] for x in outputs]).detach().mean()
        train_mmd_loss_epoch = torch.stack([x['train_mmd_loss'] for x in outputs]).detach().mean()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {'val_loss': loss, 'val_recon_loss':val_reconstruction_error, 'val_mmd_loss': mmd, 'log': logs}
    ...
    def loss_function(self, pred, true, latent):
        return (pred-true).pow(2).mean(), 0.01*self.MMD(a=torch.randn(self.hparams.batch_size, self.hparams.latent_size, requires_grad = False), b=latent)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--reg_para', default=0.01, type=int)
        return parser
...
class VaeMmdConv3Lin1XAS(VaeMmdConv3Lin1):
    ...
    def prepare_data(self):
        train_dataset = XasDataset(json_file=self.hparams.json_file,
                                root_dir='../data/',
                                transform=Compose([XasInterpolate(),XasToTensor()]))##[Normalize(), 
        self.train_size = int(0.98*len(train_dataset))
        self.val_size = len(train_dataset)-self.train_size
        self.train_ds, self.val_ds = random_split(train_dataset,[self.train_size,self.val_size])
        if self.hparams.test_json_file:
            self.test_ds = XasDataset(json_file=self.hparams.test_json_file,
                                root_dir='../data/',
                                transform=Compose([XasInterpolate(),XasToTensor()]))##[Normalize(), 
    @staticmethod
    def add_model_specific_args(parent_parser):
...
class VaeMmdConv3Lin2XAS(VaeMmdConv3Lin1XAS):
    def __init__(self, hparams):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Linear(in_features = 3*32, out_features = self.hparams.hidden_layer_1_dim),
            nn.LeakyReLU(),
            nn.Linear(in_features = self.hparams.hidden_layer_1_dim, out_features = self.hparams.hidden_layer_2_dim),
            nn.LeakyReLU(),
            nn.Linear(in_features = self.hparams.hidden_layer_2_dim, out_features = hparams.latent_size)
        )
        self.decoder = nn.Sequential(
            nn.Linear(in_features = hparams.latent_size, out_features = self.hparams.hidden_layer_2_dim),
            nn.ReLU(),
            nn.Linear(in_features = self.hparams.hidden_layer_2_dim, out_features = self.hparams.hidden_layer_1_dim),
            nn.ReLU(),
            nn.Linear(in_features = self.hparams.hidden_layer_1_dim, out_features = 3*32),
            nn.ReLU(),
            UnFlatten(),
