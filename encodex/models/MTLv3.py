...
class MTLModelv3(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.task_Crystal = create_classifier(
            self.hparams.latent_size, "Crystal", hidden_layers=2
        ).create()
    ...
    def setup(self, stage):
        train_dataset = XasMultiTaskDataset(
        ...
        if self.hparams.test_json_file:
            self.test_ds = XasMultiTaskDataset(
    ...
    def validation_epoch_end(self, outputs):
        ...
        val_recon_loss_epoch = torch.stack(
            [x["log"]["val_mtl_loss"]["train_recon_loss"] for x in outputs]
        ).mean()
        val_mmd_loss_epoch = torch.stack(
            [x["log"]["val_mtl_loss"]["train_mmd_loss"] for x in outputs]
        ).mean()
        tensorboard_logs = {
...
class MultiTaskLossWrapper_v3(nn.Module):
    ...
    def forward(self, preds, y, x):
        ...
        loss_H_fraction = torch.sum(
            precision_H_fraction * loss_H_fraction + self.log_vars[2], -1
        )

        precision_Crystal = torch.exp(-self.log_vars[3])
        loss_Crystal = torch.sum(
            precision_Crystal * loss_Crystal + self.log_vars[3], -1
        )
        ...
        loss_reconstruction = torch.sum(
            precision_recon * loss_reconstruction + self.log_vars[4], -1
        )
    ...
    def loss_function(self, pred, true, latent):
        return (pred - true).pow(2).sum(), self.MMD(
            a=torch.randn(
...
class MultiTaskLossWrapper_v4(nn.Module):
    ...
    def forward(self, preds, y, x):
        ...
        precision_CN = torch.exp(-self.log_vars[0])
        # loss_CN = torch.sum(precision_CN*loss_CN + self.log_vars[0], -1)
        loss_CN = torch.sum(precision_CN * loss_CN + 0.0 * self.log_vars[0], -1)

        precision_Dist = torch.exp(-self.log_vars[1])
        # loss_Dist = torch.sum(precision_Dist*loss_Dist + self.log_vars[1],-1)
        loss_Dist = torch.sum(precision_Dist * loss_Dist + 0.0 * self.log_vars[1], -1)

        precision_H_fraction = torch.exp(-self.log_vars[2])
        # loss_H_fraction = torch.sum(precision_H_fraction*loss_H_fraction + self.log_vars[2], -1)
        loss_H_fraction = torch.sum(
            precision_H_fraction * loss_H_fraction + 0.0 * self.log_vars[2], -1
        )

        precision_Crystal = torch.exp(-self.log_vars[3])
        # loss_Crystal = torch.sum(precision_Crystal*loss_Crystal + self.log_vars[3],-1)
        loss_Crystal = torch.sum(
            precision_Crystal * loss_Crystal + 0.0 * self.log_vars[3], -1
        )
        ...
        loss_reconstruction = torch.sum(
            precision_recon * loss_reconstruction + 0.0 * self.log_vars[4], -1
        )

        ## MMD Loss ##
        precision_mmd = torch.exp(-self.log_vars[5])
        # loss_mmd = torch.sum(precision_mmd*loss_MMD+self.log_vars[5],-1)
        loss_mmd = torch.sum(precision_mmd * loss_MMD + 0.0 * self.log_vars[5], -1)
...
class MultiTaskLossWrapper_v5(nn.Module):
    ...
    def forward(self, preds, y, x):
        ...
        wtloss_reconstruction_mmd = torch.sum(
            precision_recon * (loss_reconstruction + self.hparams.reg_para * loss_mmd)
            + torch.log(1 + self.log_vars[4].exp()),
            -1,
        )
...
class MTLModelv4(MTLModelv3):
    ...
    def __init__(self, hparams):
        ...
        if self.hparams.train_encoder:
            ...
            self.autoencoder = self._get_model()
            self.autoencoder.freeze()
        self.task_CN = create_mlp_v4(
            self.hparams.latent_size, "CN", hidden_layers=2
        ).create()
        self.task_Dist = create_mlp_v4(
            self.hparams.latent_size, "Distances", hidden_layers=2
        ).create()
        self.task_H2 = create_mlp_v4(
            self.hparams.latent_size, "H_fraction", hidden_layers=2
        ).create()
        self.task_Crystal = create_mlp_v4(
            self.hparams.latent_size, "Crystal", hidden_layers=2
        ).create()
...
class MultiTaskLossWrapper_v7(nn.Module):
    ...
    def forward(self, preds, y, x):
        ...
        wtloss_reconstruction_mmd = 1000 * (
            loss_reconstruction + self.hparams.reg_para * loss_mmd
        )
        ...
        return (
            ...
            + loss_Crystal,
            {
                ...
                "loss_weight_recon": self.log_vars[4].detach(),
                "train_mmd_loss": loss_mmd.detach(),
                "loss_weight_MMD": self.hparams.reg_para
                * self.log_vars[4].detach(),  ### Modified
                "train_loss_recon_mmd_wt": wtloss_reconstruction_mmd.detach(),  ## Combined weighted Recon+MMD Loss
            },
        )
...
class MTLModelv10(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        self.task_Crystal = create_classifier(
            self.hparams.latent_size, "Crystal", hidden_layers=2, out_features=25
        ).create()
