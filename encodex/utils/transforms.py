...
class Normalize(object):
    def __call__(self, sample):
        ...
        return pd.Series({"X": normalized_X, "Y": sample.Y})
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        return (
            torch.from_numpy(
                sample.X
            ),  ## numpy.squeeze dropped for Conv input shape match of
...
class XasToTensor(object):
    def __call__(self, sample):
        ...
        return (
            torch.from_numpy(
                sample.Abs
            ).float(),  ## numpy.squeeze dropped for Conv input shape match of
...
class XasInterpolate(object):
    def __call__(self, sample, energymesh=np.array(energymesh)):
        ...
        interpolated_abs = np.array(
            [
                np.interp(x=energymesh, xp=sample.Energy[i, :], fp=sample.Abs[i, :])
                for i in range(sample.Energy.shape[0])
            ]
        )
