...
def analyze_model(dimension, val_dl, val_size, model, device, batch_size=32):
    ...
    reconstructed_spectra, mu, sigma = (
        np.zeros((val_size, 1, 100)),
        np.zeros((val_size, dimension)),
        np.zeros((val_size, dimension)),
    )
    ...
    for index, (xdata, ydata) in enumerate(val_dl):
        ...
        if index == num_batches - 1:  ## Can be fixed...instead of dropping
            reconstructed_spectra[batch_size * index :, :, :] = (
                recon_.cpu().detach().numpy()
            )
        ...
        reconstructed_spectra[batch_size * index : batch_size * (index + 1), :, :] = (
            recon_.cpu().detach().numpy()
        )
        mu[batch_size * index : batch_size * (index + 1)] = mu_.cpu().detach().numpy()
        sigma[batch_size * index : batch_size * (index + 1)] = (
            sigma_.cpu().detach().numpy()
        )
        y_CN[batch_size * index : batch_size * (index + 1)] = (
            ydata.cpu().detach().numpy()
        )
...
def plot_histograms_mu_logvar(mu, sigma):
    ...
    ax[1].set(
        title="Histograms of $\sigma$ Latent Space",
        xlabel="$\sigma ( 10^{-3}$",
        ylabel="Counts",
    )
...
def plot_zdata(z):
    ...
    fig, ax = plt.subplots(1, 2, figsize=(15, 5))
    ax[0].set(
        title="Scatterplot(Only first and second axes) of Latent Space",
        xlabel="$z_1$",
        ylabel="$z_2$",
    )
...
def show_spectra_batch(
    sample_batched, Batch_No=None, energymesh=np.array(energymesh), **kwargs
):
...
def model_prediction_test(
    loss_latentSpace, test_dl, model_type, model_ckpt_path, latent_size, loss_df
):
    """Plot Total and Avergae Loss separately on the test data"""
    model = model_type.load_from_checkpoint(model_ckpt_path)
    model.freeze()  ##Important to not update the model
    ...
    fig = test_output(
        ...
        xlabel="Linear Space",
        ylabel="Normalized Values (a.u.)",
        savefig=False,
    )
    # savefig=f"{name_exp}_lt_size_{PARAMS['LatentSpaceSize']}_model_epoch_{PARAMS['epochs']}_20200409")
    fig.show()

    total, avg, std_error, losses = calculate_test_loss(
        model.cuda(), test_dl, bins=20
    )  ## For some reason the distribution of val_losses are not consistent.
    print(total, avg, std_error)
    loss_latentSpace.append((latent_size, (total, avg, std_error)))
    print(loss_latentSpace)
...
def calculate_test_loss(model, dl, bins=10):
    ...
    total, avg, std_error = (
        torch.sum(val_losses),
        torch.median(val_losses),
        torch.std(val_losses) / val_losses.size(0),
    )
    _ = plt.figure()
    _ = plt.hist(np.array(val_losses), bins=bins)
    _ = plt.show()
