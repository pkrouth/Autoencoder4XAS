...
class XasMultiTaskDataset(Dataset):
    ...
    def __init__(self, json_file, root_dir, descriptor="CN", transform=None, MTL=True):
        ...
        self.MTL = MTL
        self.XAS_Frame["Crystal"] = self.XAS_Frame.Crystal.apply(lambda x: x[0]).astype(
            "category"
        )
        self.categories = self.XAS_Frame.Crystal.cat.categories
        self.XAS_Frame.Crystal = self.XAS_Frame.Crystal.cat.codes
        self.XAS_Frame["MTL_Label"] = self.XAS_Frame[
            ["CN", "Distances", "H_fraction", "Crystal"]
        ].apply(lambda x: x.CN + x.Distances + [x.Crystal, x.H_fraction], axis=1)
    ...
    def __getitem__(self, idx):
        ...
        sample = pd.Series(
            {
                "Energy": np.vstack(data_Energy),
                "Abs": np.vstack(data_Abs),
                "Descriptor": data_Y,
            }
        )
        # print(sample)
    ...
    def show(self, idx):
        ...
        data = pd.Series(
            {
                "x_axis": np.array(energymesh).reshape(1, -1).squeeze(),
                "y_axis": x.numpy().squeeze(),
            }
        )
        print(data)
        _ = sns.lineplot(x="x_axis", y="y_axis", data=data)
        _ = plt.title(
            f"Sample_#{idx}_{'MTL[CN+Distances+Crystal+H_fraction]:' if self.MTL else self.descriptor}{y}"
        )
...
class MTLModel(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.task_CN = _create_mlp(
            self.hparams.latent_size, "CN", hidden_layers=2
        ).create()
        self.task_Dist = _create_mlp(
            self.hparams.latent_size, "Distances", hidden_layers=1
        ).create()
        self.task_H2 = _create_mlp(
            self.hparams.latent_size, "H_fraction", hidden_layers=2
        ).create()
        self.task_Crystal = _create_classifier(
    ...
    def training_epoch_end(self, outputs):
        ...
        train_recon_loss_epoch = (
            torch.stack([x["log"]["train_recon_loss"] for x in outputs]).detach().mean()
        )
        train_mmd_loss_epoch = (
            torch.stack([x["log"]["train_mmd_loss"] for x in outputs]).detach().mean()
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {
            ...
            "val_recon_loss": reconstruction_error,
            "val_mmd_loss": mmd,
        }
...
class MultiTaskLossWrapper(nn.Module):
    ...
    def forward(self, preds, y):
        ...
        return loss_CN + loss_Dist + loss_H_fraction + loss_Crystal, {
...
class VaeMtl2Dsc(pl.LightningModule):  ## Maybe Vae2Dsc is needed
    def __init__(self, hparams, input_dim=100, Descriptor="CN", TaskList=None):
