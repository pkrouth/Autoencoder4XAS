...
class Transfer2Exp(pl.LightningModule):
    def __init__(self, pretrained_model, model_path=None):
        super().__init__()

        self.model = pretrained_model.load_from_checkpoint(model_path)
        self.hparams = self.model.hparams
        self.model.task_CN.freeze()
        self.model.task_Dist.freeze()
        self.model.task_H2.freeze()
        self.model.task_Crystal.freeze()
    ...
    def setup(self, stage):
        ...
        self.train_size = int(0.9 * len(train_dataset))
        self.val_size = len(train_dataset) - self.train_size
        self.train_ds, self.val_ds = random_split(
            train_dataset, [self.train_size, self.val_size]
        )
