...
class XasDataset(Dataset):
    ...
    def __init__(self, json_file, root_dir, descriptor="CN", transform=None):
        ...
        if self.descriptor == "Crystal":
            self.XAS_Frame["Crystal"] = self.XAS_Frame.Crystal.apply(
                lambda x: x[0]
            ).astype("category")
...
class XasMultiTaskDatasetv3(Dataset):
    ...
    def __init__(
        ...
        if self.scale_min_max:
            self.XAS_Frame.Distances = self.XAS_Frame.Distances.apply(
                lambda x: self.scalers["scaler_Dist"]
                .transform(np.vstack(x).reshape(-1, 6))
                .squeeze()
            )
            self.XAS_Frame.CN = self.XAS_Frame.CN.apply(
                lambda x: self.scalers["scaler_CN"]
                .transform(np.vstack(x).reshape(-1, 6))
                .squeeze()
            )
        ## Need Improvement with possible One-Hot Encoding
        ...
        self.XAS_Frame["MTL_Label"] = self.XAS_Frame[
            ...
            lambda x: np.hstack((x.CN, x.Distances, x.Crystal, x.H_fraction)), axis=1
        )
    ...
    def inverse_descriptors(self, output, desc=None):
        ...
        if len(output) == 14:
            output[:6] = (
                self.scalers["scaler_CN"]
                .inverse_transform(output[:6].reshape(-1, 6))
                .squeeze()
            )
            output[6:12] = (
                self.scalers["scaler_Dist"]
                .inverse_transform(output[6:12].reshape(-1, 6))
                .squeeze()
            )
        elif len(output) == 6 and desc is "CN":
            output = (
                self.scalers["scaler_CN"]
                .inverse_transform(output.reshape(-1, 6))
                .squeeze()
            )
    ...
    def show(self, idx):
        ...
        _ = sns.lineplot(x="x_axis", y="y_axis", data=data)
        _ = plt.title(
            f"Sample_#{idx}_{'MTL[CN+Distances+Crystal+H_fraction]:' if self.MTL else self.descriptor}{self.inverse_descriptors(y.numpy())}"
        )
...
class XasNormalize_Dist_CN(object):
    ...
    def __call__(self, sample):
        ...
        x_CN, x_Dist, x_ = (
            sample.Descriptor[:6],
            sample.Descriptor[6:12],
            sample.Descriptor[12:],
        )
        x_CN *= self.CN_scale_
        x_CN += self.CN_Min_
